        ...
        Object.defineProperty(obj, key, {
            ...
            get: function reactiveGetter() {
                ...
                if (Dep.target) {
                    // 进行依赖收集
                    // 为什么需要进行依赖收集：
                    // 1、data中的数据并不是所有的地方都使用到。
                    // 2、如果数据变动所有的视图都进行更新，对性能不是很友好。
                    // 例如：有一个组件1使用到data里面的一个属性name，还有其他10个组件
                    // 如果操作了name，如果时间组件全部更新，是不优化的，
                    // 所以依赖收集就是收集这个，在进行视图更新的时候
                    dep.depend();
                    if (childOb) {
                        childOb.dep.depend();
                        if (Array.isArray(value)) {
        ...
        function updateChildren(parentElm, oldCh, newCh, insertedVnodeQueue, removeOnly) {
            ...
            while (oldStartIdx <= oldEndIdx && newStartIdx <= newEndIdx) {
                if (isUndef(oldStartVnode)) {
                    ...
                    if (isUndef(idxInOld)) { // NeFw element
